...
interface SProd {
  ...
  medidas?: string; observaciones?: string; tags?: string[]; stock?: number | null;
  subcategoria?: { _id: string; nombre: string; idExcel?: string; categoria?: { _id: string; nombre: string; idExcel?: string } };
  presentaciones?: SPres[];
}
...
export function ExportCatalogTool() {
  ...
  const [stats, setStats] = useState<{ cats: number; prods: number; pres: number } | null>(null);

  const handleExport = useCallback(async () => {
    setStatus("loading"); setProgress(10); setMessage("Consultando datos en Sanity…");
    try {
      ...
      setProgress(40); setMessage("Generando hojas del Excel…");

      const catRows: Record<string, string | number | null>[] = [];
      for (const c of cats) catRows.push({ id_categoria: c.idExcel || c._id, nombre: c.nombre, id_padre: null, orden: c.orden ?? 0, descripcion: c.descripcion || null });
      for (const sc of subcats) catRows.push({ id_categoria: sc.idExcel || sc._id, nombre: sc.nombre, id_padre: sc.categoria?.idExcel || sc.categoria?._id || null, orden: sc.orden ?? 0, descripcion: null });

      const prodRows = prods.map(p => ({
        id_producto: p.idExcel || p._id, nombre: p.nombre, descripcion: p.descripcion || "",
        categorias: p.subcategoria?.idExcel || p.subcategoria?._id || "", marca: p.marca || "Genérico",
        tags: (p.tags || []).join(", "), visible: p.visible !== false ? "TRUE" : "FALSE",
        maneja_stock: p.manejaStock ? "TRUE" : "FALSE", stock_actual: p.stock ?? "",
        permite_venta_fraccionada: p.permiteVentaFraccionada ? "TRUE" : "FALSE",
        unidad_base: p.unidadBase || "unidad", medidas: p.medidas || "", observaciones: p.observaciones || "",
      }));
      ...
      for (const p of prods) {
        for (const pr of (p.presentaciones || [])) {
          presCount++;
          presRows.push({
            id_presentacion: pr._key, id_producto: p.idExcel || p._id, nombre_producto: p.nombre,
            nombre_presentacion: pr.nombre || "", factor_conversion: pr.factorConversion ?? 1,
            precio: pr.precio ?? null, visible_en_web: pr.visibleEnWeb !== false ? "TRUE" : "FALSE",
      ...
      const tagRows = Array.from(allTags).sort().map(t => ({ nombre_tag: t, descripcion: "" }));

      setProgress(70); setMessage("Creando archivo Excel…");
      const XLSX = await import("xlsx");
      const wb = XLSX.utils.book_new();
      const addSheet = (name: string, data: Record<string, unknown>[]) => {
        if (data.length === 0) { XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([[`Sin datos`]]), name); return; }
        const ws = XLSX.utils.json_to_sheet(data);
        ws["!cols"] = Object.keys(data[0]).map(key => ({ wch: Math.min(Math.max(key.length, ...data.map(r => String((r as Record<string, unknown>)[key] ?? "").length)) + 2, 50) }));
        XLSX.utils.book_append_sheet(wb, ws, name);
      };
      addSheet("Categorias", catRows); addSheet("Productos", prodRows); addSheet("Presentaciones", presRows); addSheet("Tags", tagRows);

      setProgress(90);
      XLSX.writeFile(wb, `catalogo-comercial-victor-${new Date().toISOString().slice(0, 10)}.xlsx`);
      setProgress(100);
      setStats({ cats: catRows.length, prods: prods.length, pres: presCount });
      setStatus("done"); setMessage("Exportación completada.");
    } catch (err) {
      console.error("Export error:", err);
      setStatus("error"); setMessage(err instanceof Error ? err.message : "Error desconocido.");
    }
  }, [client]);
  ...
  return (
    <div style={s.container}>
      <div style={s.header}>
        ...
        <p style={s.subtitle}>Descarga los productos activos como .xlsx editable: Categorias, Productos (con stock), Presentaciones, Tags.</p>
      </div>
      <div style={s.card}>
        <div style={s.infoRow}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#6b7280" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="M12 16v-4" /><path d="M12 8h.01" /></svg>
          4 hojas: Categorias, Productos (con stock_actual), Presentaciones, Tags.
        </div>
        <button style={{ ...s.btn, ...s.btnPrimary, ...(status === "loading" ? s.btnDisabled : {}) }} disabled={status === "loading"} onClick={handleExport}>
          {status === "loading" ? "Exportando…" : "Exportar catálogo a Excel"}
        </button>
        {status === "loading" && <div style={s.progressWrap}><div style={{ fontSize: 13, color: "#6b7280" }}>{message}</div><div style={s.progressBar}><div style={{ ...s.progressFill, width: `${progress}%` }} /></div></div>}
        {status === "done" && stats && (
        ...
        {status === "error" && <div style={{ ...s.result, background: "#fef2f2", border: "1px solid #fecaca", color: "#991b1b" }}>❌ {message}</div>}
      </div>
    </div>
  );
}
